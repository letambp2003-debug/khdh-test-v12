...
export interface Pl1ExtractedData {
  lesson_title: string;
  subject: string;
  grade: number;
  chapter: string;
...
export class Pl1Parser {
  public static extract(buffer: Buffer, filename: string): Pl1ExtractedData {
    const raw = SourceParser.parseDocxBuffer(buffer);
    ...
    if (!lesson_title || lesson_title.length < 3) {
      ...
      if (subject === 'Khoa học tự nhiên') lesson_title = 'Tốc độ chuyển động';
      else if (subject === 'Tin học') lesson_title = 'Thuật toán tìm kiếm tuần tự';
      else lesson_title = 'Đa thức';
    }
    ...
    if (ppctMatch && ppctMatch[1]) {
      ...
      if (parts.length > 1 && ppctMatch[1].includes('-') && parts[0] < parts[parts.length - 1]) {
        for (let i = parts[0]; i <= parts[parts.length - 1]; i++) ppct.push(i);
    ...
    const yccd: Array<{ id: string; text: string; level: string }> = [];
    ...
    for (const line of lines) {
      ...
      if (isBullet && hasActionVerb && line.length >= 15) {
        ...
        if (/nhận biết|nêu được|chỉ ra|mô tả|phát biểu|liệt kê/.test(lLower)) level = 'NHAN_BIET';
        else if (/vận dụng|thực hành|giải quyết|tính toán|chứng minh/.test(lLower)) level = 'VAN_DUNG';
    ...
    if (yccd.length === 0) {
      yccd.push(
        { id: 'YCCD-01', text: `Nhận biết được khái niệm cốt lõi của bài học ${lesson_title}.`, level: 'NHAN_BIET' },
        { id: 'YCCD-02', text: `Thông hiểu và giải thích được quy tắc bài học ${lesson_title}.`, level: 'THONG_HIEU' },
        { id: 'YCCD-03', text: `Vận dụng được kiến thức bài ${lesson_title} vào bài toán thực tiễn.`, level: 'VAN_DUNG' }
      );
    }
    ...
    const primaryItem: LessonCatalogItem = {
      id: 'cat_item_01',
      lesson_title,
      subject,
      grade,
  ...
  public static getStandardCurriculum(subject: string, grade: number): LessonCatalogItem[] {
