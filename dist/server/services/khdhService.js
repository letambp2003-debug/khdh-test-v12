...
class KhdhService {
     ...
     */
    static async generateOneShot(lesson, options = {}) {
...
4. Tiến trình 4 phần:
   ...
   - D. Vận dụng & Hướng dẫn về nhà (4 mục: Ghi nhớ, Hoàn thành, Chuẩn bị, Tự học)`;
        ...
        const aiRes = await geminiService_1.GeminiService.generateWithThinking(prompt, systemInstruction, model, budget);
     ...
     */
    static async generateStep(step, lesson, options = {}) {
        const model = options.model || 'gemini-3.7-flash';
        const budget = options.thinkingBudget || 8192;
        const stepTitles = [
            '',
...
Hãy thiết kế chi tiết nội dung chuẩn sư phạm GDPT 2018 theo đúng bước này.`;
        const aiRes = await geminiService_1.GeminiService.generateWithThinking(prompt, 'Tập trung sâu vào hoạt động của bước yêu cầu.', model, budget);
        let stepActivities = [];
        if (step === 1) {
            stepActivities = [
                {
     ...
     */
    static syncArtifacts(lesson) {
        // Đánh dấu cờ STALE cho các artifact để sẵn sàng tái tạo
        dependencyGraph_1.DependencyGraphService.markDependentsStale('act_b2');
        return {
            worksheetData: {
...
exports.KhdhService = KhdhService;
